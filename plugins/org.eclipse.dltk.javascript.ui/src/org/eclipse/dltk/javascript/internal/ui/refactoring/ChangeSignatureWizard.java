...
public class ChangeSignatureWizard extends RefactoringWizard {
	...
	private static class ChangeSignatureInputPage extends UserInputWizardPage {
		...
		private Control createParameterTableControl(Composite composite) {
			...
			}, ChangeParametersControl.Mode.CHANGE_METHOD_SIGNATURE/*, fProcessor.getStubTypeContext()*/);
...
//			ViewForm border= new ViewForm(composite, SWT.BORDER | SWT.FLAT);
			...
			fSignaturePreview= new ScriptSourceViewer(composite, null, null, false, SWT.READ_ONLY | SWT.V_SCROLL | SWT.WRAP /*| SWT.BORDER*/, store);
			fSignaturePreview.configure(new JavascriptSourceViewerConfiguration(toolkit.getTextTools().getColorManager(), store, null, null));
